...
class GLEU:
    ...
    def load_hypothesis_sentence(self, hypothesis):
        ...
        self.this_h_ngrams = [
            self.get_ngram_counts(hypothesis, n) for n in range(1, self.order + 1)
        ]

    def load_sources(self, spath):
        """load n-grams for all source sentences"""
        self.all_s_ngrams = [
            [self.get_ngram_counts(line.split(), n) for n in range(1, self.order + 1)]
    ...
    def gleu_stats(self, i, r_ind=None):
        ...
        for n in range(1, self.order + 1):
            ...
            yield max(
                [
                    sum((h_ngrams & r_ngrams).values())
                    - sum((h_ngrams & s_ngram_diff).values()),
                    0,
                ]
            )
    ...
    def run_iterations(
        ...
        for j in range(num_iterations):
            random.seed(j * 101)
            indices.append(
                [random.randint(0, self.num_refs - 1) for i in range(len(hyp))]
            )
                ...
                if debug:
                    print(
                        ...
                        " ".join(
                            self.get_gleu_stats(
                                [
                                    self.gleu(stats, smooth=True)
                                    for stats in stats_by_ref
                                ]
                            )
                        ),
                    )
                yield self.get_gleu_stats(
                    [self.gleu(stats, smooth=True) for stats in stats_by_ref]
                )
        if not per_sent:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sent", default=False, action="store_true", help="sentence level scores"
    )
